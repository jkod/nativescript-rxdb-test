...
function WebSqlPouch(opts, callback) {
  ...
  async function dbExecuteSql(sql, args, cb) {
    // console.log(sql, args);
    // try {
      if( sql.startsWith('SELECT') ) {
        console.log('SELECT CALL:', sql);
        const result = await db.select(sql, args);
        console.log(result);
        cb(db, result);
      } else if( sql.startsWith('INSERT')) {
        console.log('INSERT CALL:', sql);
        const x = await db.execute(sql, args);
        let result = await db.select("SELECT last_insert_rowid()");
        result = result[0]['last_insert_rowid()'];
        console.log('new result insert', result);
        cb(db, result);
      } else {
        console.log('OTHER CALL:', sql);
  ...
  function checkEncoding(tx, cb) {
    ...
    dbExecuteSql('SELECT HEX("a") AS hex', [], function (tx, res) {
        var hex = res[0].hex;
  ...
  function onGetVersion(tx, dbVersion) {
    ...
    if (dbVersion === 0) {
      ...
      dbExecuteSql(attach);
      dbExecuteSql(local);
      dbExecuteSql(attachAndRev, [], function () {
        dbExecuteSql(ATTACH_AND_SEQ_STORE_SEQ_INDEX_SQL);
        dbExecuteSql(ATTACH_AND_SEQ_STORE_ATTACH_INDEX_SQL);
      });
      dbExecuteSql(doc, [], function () {
        dbExecuteSql(DOC_STORE_WINNINGSEQ_INDEX_SQL);
        dbExecuteSql(seq, [], function () {
          dbExecuteSql(BY_SEQ_STORE_DELETED_INDEX_SQL);
          dbExecuteSql(BY_SEQ_STORE_DOC_ID_REV_INDEX_SQL);
          dbExecuteSql(meta, [], function (tx, result) {
            ...
            dbExecuteSql(initSeq, initSeqArgs, function (tx, result) {
      ...
      var setupDone = function () {
        ...
        if (migrated) {
          ...
          dbExecuteSql('UPDATE ' + META_STORE + ' SET db_version = ' +
            ADAPTER_VERSION);
        }
  ...
  function fetchVersion(tx) {
    ...
    dbExecuteSql(sql, [], function (tx, result) {
      console.log('[pdb-index]fetchVersion Select result:', tx, result);
      if (result?.length == 0 || !result) {
        ...
        // so add it.
        dbExecuteSql('ALTER TABLE ' + META_STORE +
        ...
        dbExecuteSql('SELECT db_version FROM ' + META_STORE,
          [], function (tx, result) {
          console.log(result);
  ...
  function getMaxSeq(tx, callback) {
    console.log("[pdb-index]", "getMaxSeq." );
  ...
  api._changes = function (opts) {
    ...
    function fetchChanges() {
      ...
      db.readTransaction(() => {
        dbExecuteSql(sql, sqlArgs, function (db, result) {
      ...
      }).catch(websqlError(opts.complete)).then(function () {
        if (!opts.continuous) {
          opts.complete(null, {
            results: results,
            last_seq: lastSeq
  ...
  api._doCompaction = function (docId, revs, callback) {
    ...
    db.transaction(() => {
      ...
      dbExecuteSql(sql, [docId], function (db, result) {
        ...
        dbExecuteSql(sql, [safeJsonStringify(metadata), docId]);
      });
  ...
  api._getLocal = function (id, callback) {
    ...
    db.readTransaction(() => {
      ...
      dbExecuteSql(sql, [id], function (db, res) {
        if (res.length) {
          var item = res[0];
  ...
  api._removeLocal = function (doc, opts, callback) {
    ...
    if (opts.ctx) {
      ...
      db.transaction(() => {removeLocal()}).catch(websqlError(callback)).then(function () {
        if (ret) {
          callback(null, ret);
        }
      });
  ...
  api._destroy = function (opts, callback) {
    ...
    db.transaction(() => {
      ...
      stores.forEach(function (store) {
        dbExecuteSql('DROP TABLE IF EXISTS ' + store, []);
      });
    }).catch(websqlError(callback)).then(function () {
      if (hasLocalStorage()) {
